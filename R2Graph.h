...
class R2Vector
{
  public:
    ...
    R2Vector(double xx, double yy) : x(xx), y(yy) {}
    ...
    R2Vector operator*(double c) const { return R2Vector(x * c, y * c); }
    ...
    {
        //... return (x == v.x && y == v.y);
        return (fabs(x - v.x) <= R2GRAPH_EPSILON &&
    ...
    double signed_area(const R2Vector &v) const { return (x * v.y - y * v.x); }
...
class R2Point
{
  public:
    ...
    R2Point(double xx, double yy) : x(xx), y(yy) {}
    ...
    R2Point operator*(double c) const { return R2Point(x * c, y * c); }
    ...
    {
        //... return (x == p.x && y == p.y);
        return (fabs(x - p.x) <= R2GRAPH_EPSILON &&
    ...
    static double signed_area(const R2Point &a, const R2Point &b,
                              const R2Point &c)
    {
    ...
    static double angle(const R2Point &A, const R2Point &B, const R2Point &C)
    {
        return A.angle(B, C);
    }

    double distance(const R2Point &p) const { return (p - *this).length(); }
...
class R2Rectangle
{
    ...
    double h; // height
  public:
    R2Rectangle() : l(0.), b(0.), w(0.), h(0.) {}

    R2Rectangle(double _left, double _bottom, double _width, double _height)
        : l(_left), b(_bottom), w(_width), h(_height)
    {
    }

    R2Rectangle(const R2Point &_leftBottom, double _width, double _height)
        : l(_leftBottom.x), b(_leftBottom.y), w(_width), h(_height)
    {
    }

    R2Rectangle(const R2Rectangle &r) : l(r.l), b(r.b), w(r.w), h(r.h) {}
    ...
    {
        return (l <= p.x && p.x < l + w && b <= p.y && p.y < b + h);
    }
    ...
    bool clip(const R2Point &p1, const R2Point &p2, R2Point &c1,
              R2Point &c2) const
    {
        c1 = p1;
        c2 = p2;
...
class I2Vector
{
  public:
    ...
    I2Vector(const I2Vector &v)
        : // Copy-constructor
          x(v.x), y(v.y)
    {
    }

    I2Vector(int xx, int yy) : x(xx), y(yy) {}
    ...
    I2Vector operator*(int c) const { return I2Vector(x * c, y * c); }
    ...
    int operator*(const I2Vector &v) const
    { // Scalar product
        return x * v.x + y * v.y;
    }
    ...
    I2Vector normal() const
    { // Normal to this vector
        return I2Vector(-y, x);
    }

    double angle(const I2Vector &v) const
    { // Angle from this vector to v
        double xx = v * (*this);
        double yy = v * normal();
        return atan2(yy, xx);
    ...
    // Comparings
    bool operator==(const I2Vector &v) const { return (x == v.x && y == v.y); }
...
class I2Point
{
  public:
    ...
    I2Point(const I2Point &p)
        : // Copy-constructor
          x(p.x), y(p.y)
    {
    }

    I2Point(int xx, int yy) : x(xx), y(yy) {}
    ...
    I2Point operator*(int c) const { return I2Point(x * c, y * c); }
    ...
    // Comparings
    bool operator==(const I2Point &p) const { return (x == p.x && y == p.y); }
...
class I2Rectangle
{
  ...
  public:
    I2Rectangle() : l(0), t(0), w(0), h(0) {}

    I2Rectangle(int _left, int _top, int _width, int _height)
        : l(_left), t(_top), w(_width), h(_height)
    {
    }

    I2Rectangle(const I2Point &_leftTop, int _width, int _height)
        : l(_leftTop.x), t(_leftTop.y), w(_width), h(_height)
    {
    }

    I2Rectangle(const I2Rectangle &r) : l(r.l), t(r.t), w(r.w), h(r.h) {}
    ...
    {
        return (l <= p.x && p.x < l + w && t <= p.y && p.y < t + h);
    }
...
// Global functions
bool intersectLineSegments(const R2Point &p0,
                           const R2Point &p1, // First line segment
                           const R2Point &q0,
                           const R2Point &q1, // Second line segmant
                           R2Point &intersection // Result
);

bool intersectLineSegmentAndLine(const R2Point &p0,
                                 const R2Point &p1, // Line segment
                                 const R2Point &q,
                                 const R2Vector &v, // Straight line
                                 R2Point &intersection // Result
);
...
{
    return intersectLineSegmentAndLine(p0, p1, q0, (q1 - q0), intersection);
}

bool intersectStraightLines(const R2Point &p, const R2Vector &v, // First line
                            const R2Point &q, const R2Vector &w, // Second line
                            R2Point &intersection // Result
);

inline bool intersectStraightLines(const R2Point &p0,
                                   const R2Point &p1, // First line
                                   const R2Point &q0,
                                   const R2Point &q1, // Second line
                                   R2Point &intersection // Result
)
{
    return intersectStraightLines(p0, (p1 - p0), q0, (q1 - q0), intersection);
}
